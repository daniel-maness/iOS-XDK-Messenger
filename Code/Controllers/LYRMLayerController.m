...
NSString *const LYRMConversationMetadataDidChangeNotification = @"LSConversationMetadataDidChangeNotification";
NSString *const LYRMConversationParticipantsDidChangeNotification = @"LSConversationParticipantsDidChangeNotification";
NSString *const LYRMConversationDeletedNotification = @"LSConversationDeletedNotification";
NSString *const LYRMLayerControllerErrorDomain = @"LYRMLayerControllerErrorDomain";

@interface LYRMLayerController ()

@property (nonnull, nonatomic, readwrite) id<LYRMAuthenticating> authenticationProvider;
...
@implementation LYRMLayerController
...
- (id)initWithLayerAppID:(nonnull NSURL *)layerAppID clientOptions:(nullable LYRClientOptions *)clientOptions authenticationProvider:(nonnull id<LYRMAuthenticating>)authenticationProvider
{
    self = [super init];
    if (self) {
        _layerClient = [LYRClient clientWithAppID:layerAppID delegate:self options:clientOptions];
        _layerClient.debuggingEnabled = YES;
        _layerConfiguration = [[LYRUIConfiguration alloc] initWithLayerClient:_layerClient];
...
- (void)authenticateWithCredentials:(LYRMUserCredentials *)credentials completion:(void (^)(LYRSession *session, NSError *error))completion
{
...
- (void)layerClient:(LYRClient *)client objectsDidChange:(NSArray *)changes
{
    for (LYRObjectChange *change in changes) {
        ...
        if (change.type == LYRObjectChangeTypeUpdate && [change.property isEqualToString:@"metadata"]) {
            [[NSNotificationCenter defaultCenter] postNotificationName:LYRMConversationMetadataDidChangeNotification object:change.object];
        }
        if (change.type == LYRObjectChangeTypeUpdate && [change.property isEqualToString:@"participants"]) {
            [[NSNotificationCenter defaultCenter] postNotificationName:LYRMConversationParticipantsDidChangeNotification object:change.object];
        }
        if (change.type == LYRObjectChangeTypeDelete) {
            [[NSNotificationCenter defaultCenter] postNotificationName:LYRMConversationDeletedNotification object:change.object];
        }
    }
}
