...
#import "LYRMAuthenticationViewController.h"
#import "LYRMTestUser.h"
#import "LYRMUtilities.h"

@interface LYRMPushNotificationTest : KIFTestCase
...
@implementation LYRMPushNotificationTest

- (void)setUp
{
    [super setUp];
    
    LYRMLayerController *applicationController =  [(LYRMAppDelegate *)[[UIApplication sharedApplication] delegate] applicationController];
    ...
    [self.testInterface registerAndAuthenticateTestUser:[LYRMTestUser testUserWithNumber:10]];
...
- (NSMutableArray *)instantiateLayerClientsWithCount:(NSUInteger)count
{
    ...
    for (int i = 0; i < count; i++) {
        LYRClient *layerClient = [LYRClient clientWithAppID:LYRMLayerAppID(self.testInterface.testEnvironment)];
        LYRMTestUser *testUser = [LYRMTestUser testUserWithNumber:i];
