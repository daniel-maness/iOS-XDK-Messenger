...
@interface LYRMTestInterface ();

@end

@implementation LYRMTestInterface
...
- (id)initWithApplicationController:(LYRMLayerController *)applicationController
{
    self = [super init];
    if (self) {
        ...
        _contentFactory = [LYRMLayerContentFactory layerContentFactoryWithLayerClient:applicationController.layerClient];
    }
    return self;
}
...
- (LYRMUser *)randomUser
{
    ...
    LYRMUser *user = [[users allObjects] objectAtIndex:randomNumber];
...
{
    LYRMUser *user = [self.applicationController.persistenceManager userForIdentifier:identifier];
...
- (NSString *)conversationLabelForParticipants:(NSSet *)participantIDs
{
    ...
    LYRMUser *firstUser = [[participants allObjects] objectAtIndex:0];
    ...
    for (int i = 1; i < [[participants allObjects] count]; i++) {
        LYRMUser *user = [[participants allObjects] objectAtIndex:i];
